...
filePath <- "C:/Richa/Internships/Quantium/Task2/"
...
# Creating a function to calculate correlation for a measure, looping through each control store
...
# Creating a combined score composed of correlation and magnitude, by first merging the correlations table with the magnitude table
...
# Selecting control stores based on the highest matching store (closest to 1 but not the store itself, i.e. the second ranked highest store)
...
# As there are 8 months in the pre-trial period hence 8 - 1 = 7 degrees of freedom
degreesOfFreedom <- 7
...
# to check whether the hypothesis is statistically significant. The test statistic here is (x - u)/standard deviation
...
# As our null hypothesis is that the trial period is the same as the pre-trial period, let's take the standard deviation based on the scaled percentage difference
...
#### Trial and control store total sales
# Create a table with sales by store type and month. We only need data for the trial and control store.
measureOverTimeSales <- measureOverTime
pastSales <- measureOverTimeSales[, Store_type := ifelse(STORE_NBR == trial_store, "Trial",
                                                         ifelse(STORE_NBR == control_store, "Control", "Other stores"))
][, totSales := mean(totSales), by = c("YEARMONTH", "Store_type")
...
# Calculate the 5th and 95th percentile for control store sales. The 5th and 95th percentiles can be approximated by using two standard deviations away from the mean.
# The variable stdDev earlier calculates standard deviation in percentages, and not dollar sales.
...
# Visual checks on trends based on the drivers. For the period before the trial, create a graph with total sales of the trial 
...
# Visual checks on trends based on the driver. For the period before the trial, create a graph with customer counts of the
...
# As our null hypothesis is that the trial period is the same as the pre-trial period, let's take the standard deviation based on the scaled percentage 
